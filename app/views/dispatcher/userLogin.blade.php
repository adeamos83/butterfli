					...
					<img src="<?php echo asset_url(); ?>/web/img/form-logo.png" alt="form-logo" />
						...
						<ul>
							...
							<li class="half" id="create_account">
								<a href="{{route('/dispatcher/signup')}}">Create an account</a>
							</li>
							<li>
								<div id="error_messages" style="display:none;" class="alert alert-danger">
...
<script type="text/javascript">
	function userVerify(){
        ...
        if($('#email_check1').val()=='' || $('#password').val()==''){
            ...
            $.ajax({
                type: "POST",
                url:'<?php echo URL::Route('/dispatcher/verify') ?>',
                data:{email:$('#email_check1').val(),password:$('#password').val()},
...
//                        $("#show_otp_form").trigger("click");
	...
	function checkOTP(){
        if($('#otp').val()==''){
            ...
            $.ajax({
                type: "POST",
                url:'<?php echo URL::Route('CheckUserOTP') ?>',
                data:{otp:$('#otp').val(),email:$('#email_check1').val()},
                success: function(data) {
                    console.log(data);
                    if(data==1){
                        Ladda.create( document.querySelector( '.submit_otp' ) ).stop();
	...
	function resendOTP(){
		...
		$.ajax({
			type: "POST",
			url:'<?php echo URL::Route('ResendOTP') ?>',
			data:{email:$('#email_check1').val()},
			success: function(data) {
				console.log(data);
				$('#loadingimage1').hide();
				$('#user_email').val(data);
