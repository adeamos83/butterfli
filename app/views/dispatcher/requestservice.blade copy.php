					...
					<div>Healthcare Providers</div>
						<span class=" bg_arrow" >
							<select style="height:30px;background-color:#ffffff;width: 50%; border:1px solid rgba(10,120,177,0.4); " name="company_name" id="company_name" onchange="company_name_select(this);">
								<option selected="true" value="0" style="width:90%;">Select Company Name</option>
				...
				<div id="roundtripdiv" style="display:none;" class="form-group row">
					<div class="col-sm-6 col-xs-12" style="font-size:12px;">
						...
						<div class="form-group row">
							<div class="col-sm-6 col-xs-12" style="font-size:12px;">
								<div>Pickup address</div>
								<div id="round_pickupaddress">

							</div>
							<div class="col-sm-6 col-xs-12" style="font-size:12px;">
								<div>Dropoff address</div>
								<div id="round_dropoffaddress">
							</div>
						</div>
						<div class="form-group row">
						...
						<div id="roundtrip_attendant" style="display: none;">
							<div class="form-group row">
								<div class="col-sm-6 col-xs-12" style="font-size:12px;">
									<div>Attendant Name</div>
									<input type="text" name="round_attendant_name" id="round_attendant_name"
										   placeholder="" value="">

								</div>
								<div class="col-sm-6 col-xs-12" style="font-size:12px;">
									<div>Phone</div>
									<span style="vertical-align: -9px;font-size: 15px;width:16%">+1</span> <input type="text" name="round_attendant_phone" id="round_attendant_phone" style="width: 84%;position: absolute;" placeholder=""
											  value="" >
								</div>
							</div>
							<div class="form-group row">
								<div class="col-sm-12 col-xs-12" style="font-size:12px;">
									<div>Attendant Pickup address</div>
									<input type="text" name="round_attendant_pickupaddress" id="round_attendant_pickupaddress" placeholder=""
										   value="" >
									<span class="round_attendantstart">
                                        <input type="hidden" name="round_attendantstart_lat" data-geo="lat">
                                        <input type="hidden" name="round_attendantstart_lng" data-geo="lng">
                                    </span>
								</div>
							</div>
						</div>
			...
			<div class="col-md-6 col-sm-6"> 
				...
				<div class="col-md-12 col-sm-12">
					...
					<div id="estimate_cost" style="display:none;" class="form-group row">
						<div class="col-sm-12 col-md-12 col-xs-12" style="font-size:12px;">
							<h2 style="text-align:center;border-bottom:1px solid#000;line-height:0.3em;margin: 10px 0 20px;"><span style="background:#fff;"><strong style="font-size:15px;">Estimated Fare Breakdown</strong></span></h2>
							<br>
        ...
        distanceService.getDistanceMatrix({
            ...
            function (response, status) {
                if (status !== google.maps.DistanceMatrixStatus.OK) {
                    ...
                    //alert(response.rows[0].elements[0].duration.value);
                    var durationInMinutes = response.rows[0].elements[0].duration.value / 60;
                    var distanceInMiles = response.rows[0].elements[0].distance.value / (1000 * 1.609344);
                    ...
                    if(response.rows[0].elements[0].status == 'OK'){
                        ...
                        $.ajax({
                            ...
                            data: {distance: distanceInMiles, duration: durationInMinutes, type:$('#services').val(), is_wheelchair:wheelchair,is_oxygen_mask:oxygen_mask,is_roundtrip:roundtrip},
                            success: function (data) {
                                console.log(data);
                                if (data != '') {
                                    ...
                                    $('#baseprice').text(formatter.format(data.base_price));
                                    $('#distance').text(data.distance);
                                    var mileage = formatter.format(data.mileage);
                                    var new_mileage = mileage+"/mile";
                                    $('#mileage').text(new_mileage);
                                    showHideWheelChairElements(wheelchair, formatter.format(data.wheelchair_cost));
                                    showHideOxygenMaskElements(oxygen_mask, formatter.format(data.oxygen_mask_price));
                                    $('#amount').text(formatter.format(data.showamount));
                                    $('#distance_db').val(data.distance);
                                    $('#time_db').val(data.duration);
                                    $('#total_db').val(data.amount);
                                    $('#total_roundtrip_amount').val(data.roundamount);
                                    $('#estimate_cost').show();
                                }else{
//										alert("hi");
                                }
	...
	function validate_fields(){
        ...
        if(ride_type==1){
            ...
            if($('#passenger_contact_name').val()=='' || $('#passenger_phone').val()=='' || $('#geocomplete').val()=='' || $('#dropoff').val()=='' ){
                ...
                var roundtrip = + $('#roundtrip').is(':checked');
                if(attendant==1){
                    if($('#attendant_pickupaddress').val()=='' || $('#attendant_name').val()=='' || $('#attendant_phone').val()==''){
                        alert("Please fill all attendant details");
                        return false;
                    }
                }
                if(roundtrip==1 && attendant==1){
                    if($('#round_attendant_name').val()=='' || $('#round_attendant_phone').val()=='' || $('#round_attendant_pickupaddress').val()==''){
                        alert("Please fill all attendant details for round trip");
                        return false;
                    }
				}
				//alert(payment_type);
