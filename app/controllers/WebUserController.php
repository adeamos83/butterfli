...
class WebUserController extends \BaseController {
    ...
    public function __construct() {
        ...
        $this->beforeFilter(function() {
            if (!Session::has('user_id')) {
                ...
                Session::put('user_name', $owner->contact_name);
    ...
    public function BookFromWeb() {
        return View::make('web.bookfromweb');
    }
    ...
    public function saveUserReview() {
        ...
        $request = RideRequest::where('id', $request_id)->where('owner_id', $owner_id)->first();
        $rating = 0;
        if (Input::has('rating')) {
    ...
    public function userLogin() {
        Response::header('X-Frame-Options', ' ALLOW FROM https://gobutterfli.com');
        Response::header('X-Frame-Options', ' ALLOW FROM https://www.gobutterfli.com');
    ...
    public function userTripCancel() {
        ...
        if ($request->owner_id == $owner_id) {
          RideRequest::where('id', $request_id)->update(array('is_cancelled' => 1));
    ...
    public function userTripStatus() {
        ...
        $owner_id = Session::get('user_id');
        $request = RideRequest::where('id', $id)->first();
        if ($request != NULL) {
            $status = 0;
    ...
    public function paypalstatus() {
        $request =RideRequest::find(Session::get('request_id'));
    ...
    public function request_fare() {
        ...
        /* $json_resp = file_get_contents('https://maps.googleapis.com/maps/api/distancematrix/json?origins=' . $latitude . ',' . $longitude . '&destinations=' . $d_latitude . ',' . $d_longitude); */
    ...
    public function userSave() {
        ...
        if ($validator->fails()) {
            ...
            if (Owner::where('email', $email)->count() == 0) {
                ...
                $owner->save();
                regenerate:
                $referral_code1 = my_random6_number();
                if (Ledger::where('referral_code', $referral_code1)->count()) {
                    goto regenerate;
                }
                /* Referral entry */
                $ledger = new Ledger;
                ...
                return Redirect::to('booking/signin')->with('success', 'You have successfully registered. <br>Please Login');
            } else {
    ...
    public function userForgotPassword() {
        ...
        if ($owner) {
            ...
            return Redirect::to('booking/signin')->with('success', 'password reseted successfully. Please check your inbox for new password.');
        } else {
    ...
    public function userTrips() {
        $owner_id = Session::get('user_id');
        $requests = RideRequest::where('owner_id', $owner_id)
                ...
                ->select('request.id', 'request_start_time', 'walker.contact_name', 'request.total as total', 'walker_type.name as type')
                ->get();
    ...
    public function userScheduledTrips() {
        $owner_id = Session::get('user_id');
        $requests = ScheduledRequest::where('scheduled_requests.owner_id', $owner_id)
                ->leftJoin('owner', 'scheduled_requests.owner_id', '=', 'owner.id')
                ->orderBy('scheduled_requests.server_start_time', 'ASC')
                ->select(
                        'owner.contact_name as owner_contact_name', 'owner.id as owner_id', 'scheduled_requests.id as id', 'scheduled_requests.created_at as date', 'scheduled_requests.time_zone', 'scheduled_requests.src_address', 'scheduled_requests.dest_address', 'scheduled_requests.promo_code', 'scheduled_requests.start_time', 'scheduled_requests.start_time', 'scheduled_requests.payment_mode')
                ->get();
        $total_schedules = ScheduledRequest::where('owner_id', '=', $owner_id)->count();
        /* $var = Keywords::where('id', 4)->first(); */

        return View::make('web.userScheduledTrips')
                        /* ->with('title', 'My ' . $var->keyword . 's') */
                        ->with('title', 'My Scheduled ' . Config::get('app.generic_keywords.Trip') . 's' . " : Total = " . $total_schedules)
                        ->with('requests', $requests);
    }
    ...
    public function delete_future_request() {
        ...
        return Redirect::to("/user/scheduledtrips");
    }
    ...
    public function updateUserProfile() {
        ...
        $validator = Validator::make(
                    ...
                    /* 'picture' => 'required|mimes:jpeg,bmp,png' */
                    'user_id' => 'Your unique id is missing.',
                        )
        );
    ...
    public function surroundingCars() {
        if ($_GET) {
            ...
            foreach ($walkers as $key) {
                ...
                $walker_d[$inc][1] = $key->contact_name;
