...
class WebController extends \BaseController {
	...
	public function privacypolicy() {
        ...
        $app_name = Config::get('app.website_title');
        return View::make('website.privacypolicy')
        ...
        if ($provider->merchant_id == NULL) {
            if (Config::get('app.default_payment') == 'stripe') {
                return View::make('website.banking_provider_stripe')
                                ...
                                ->with('provider_id', $id)
                                ->with('provider_contact_name', $provider_contact_name)
    ...
    public function track_ride() {
        $id = Request::segment(2);
        $request = RideRequest::where('security_key', $id)->where('is_started', 1)->where('is_completed', 0)->first();
        if ($request) {
            $owner = Owner::where('id', $request->owner_id)->first();
            $user_name = $owner->contact_name;
            $theme = Theme::all();
            $logo = '/image/logo.png';
            $favicon = '/image/favicon.ico';
            foreach ($theme as $themes) {
