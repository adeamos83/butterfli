...
class SchedulePush extends Command {
	...
	public function fire()
	{
		...
		$requests = RideRequest::where('later',1)->where('is_started',0)->where('request_start_time','<=',$msgtime)->get();
		if($requests->count()>0){
			$this->info('hola');
		foreach ($requests as $request) {
			$provider=Walker::where('id',$request->confirmed_walker)->first();
			$owner=Owner::where('id',$request->owner_id)->first();
			$message="You have a ride scheduled in 30 mins from now. Client name is $owner->contact_name and phone no. is $owner->phone";
